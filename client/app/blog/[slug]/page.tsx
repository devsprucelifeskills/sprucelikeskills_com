    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="relative pt-32 pb-24 md:pt-48 md:pb-32 bg-[#0A3D24] overflow-hidden">
        ...
        <div className="container mx-auto px-4 max-w-4xl relative z-10 text-center">
          <Link href="/blog" className="inline-flex items-center gap-2 text-[#13523f] font-black text-xs uppercase tracking-widest mb-10 hover:translate-x-[-4px] transition-transform">
          ...
          <div className="flex items-center justify-center gap-6 p-6 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 max-w-md mx-auto">
            <div className="w-12 h-12 bg-[#13523f] rounded-full flex items-center justify-center font-black text-white">
              SE
            </div>
        ...
        <div className="prose prose-xl prose-slate max-w-none prose-p:text-gray-700 prose-p:leading-[1.8] prose-a:text-[#0A3D24] prose-strong:text-gray-900 prose-strong:font-bold">
          {post.content.split('\n').filter(p => p.trim() !== '').map((block: string, idx: number) => {
            ...
            if (trimmedBlock.startsWith('###')) {
              return (
                <h3 key={idx} className="text-2xl md:text-3xl font-bold text-gray-900 mt-16 mb-8 border-l-4 border-[#13523f] pl-6">
            ...
            if (trimmedBlock.startsWith('-')) {
              return (
                <li key={idx} className="flex items-start gap-4 text-lg font-medium text-gray-700 leading-relaxed mb-4 list-none">
                  <span className="w-2 h-2 bg-[#13523f] rounded-full mt-2.5 shrink-0" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {relatedPosts.map((rPost) => (
              ...
              >
                ...
                <div className="p-8">
                  <div className="text-[10px] font-black uppercase tracking-[0.2em] text-[#13523f] mb-3">{rPost.category}</div>
      ...
      <section className="py-24 bg-[#0A3D24] relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 max-w-4xl text-center relative z-10">
          <h2 className="text-3xl md:text-5xl font-black text-white mb-10 leading-tight">
            Inspired by <span className="text-[#13523f]">Our Vision?</span>
          </h2>
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-6">
            <button className="bg-[#13523f] text-white font-black px-12 py-5 rounded-full uppercase tracking-widest text-sm shadow-2xl hover:bg-[#1a6e4a] transition-all transform hover:-translate-y-1">
              Start Your Journey
            </button>
